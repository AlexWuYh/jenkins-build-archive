...
独立于 Jenkins 的构建历史元数据归档：在 Build Record 被清理后，仍可追溯 Job、Git 分支/Commit、Docker Image Tag/Digest。

## 目标

- 接收 Jenkins 推送的构建元数据并持久化（SQLite）
- `(jobName, buildId)` 幂等写入
- Web UI + REST 查询、筛选、分页、详情
- 管理端删除与可配置保留策略
...
| 写接口 | `POST /api/v1/builds` 及管理类 DELETE/retention |
...
3. 密钥仅环境变量；默认 token 占位必须 fail-closed。
